...
/*   Updated: 2022/11/10 20:03:03 by imittous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_routin(t_philz **info)
{
	...
	while (1)
	{
		...
		printf ("%lld %d is thinking\n", (ft_get_time()
				- (*info)->f_dta->reference), (*info)->id + 1);
		sem_post((*info)->f_dta->print);
		if ((*info)->f_dta->check_eat == (*info)->f_dta->philomusteat)
			exit(0);
	}
	return (0);
}
...
int	main(int ac, char **av)
{
	...
	if (!info)
		return (0);
	ft_parsing(av, &info, ac, &philo_info);
	sem_unlink("/sem_fork");
	...
	{	
		...
		philo_info->last_meal = ft_get_time() + info->tmtdie;
